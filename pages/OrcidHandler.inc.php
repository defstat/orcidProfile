...
class OrcidHandler extends Handler {
	...
	function orcidAuthorize($args, $request) {
		...
		$curl_url = $plugin->getSetting($contextId, 'orcidProfileAPIPath') . OAUTH_TOKEN_URL;
		...
		if (!($result = curl_exec($curl))) {
			error_log('ORCID CURL error: ' . curl_error($curl) . ' (' . __FILE__ . ' line ' . __LINE__ . ', URL ' . $curl_url . ')');
				...
				if (!is_null($response)) {
					...
					if (is_null($users) || $users->count == 0) { // If no user exists
						...
						if (is_null($users) || $users->count == 0) { // If no user exists
							...
							// get all emails
							$emails = $response['orcid-profile']['orcid-bio']['contact-details']['email'];
					...
					echo '
					<html><body><script type="text/javascript">
						opener.document.getElementById("orcidLoginMessage").innerText = "' . __('plugins.generic.oauth.message.oauthLoginError') . '";
						window.close();
						</script></body></html>
					';
